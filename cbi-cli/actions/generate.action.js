...
import { url, str, logger } from "../lib/utils/index.js";
...
const generateFiles = (inputs) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (!name) {
            ...
            throw new Error(t('Please input shcematic name'))
        }
        let generateFile = []
        ...
        //     case '': 
        // }
        let schematic = CbiCollection.schematics.find(e => e.name === inputs.type || e.alias === inputs.type)
        if (schematic) {
            schematic.run({ ...inputs, name })

        } else {
            throw new Error(t('Schematic not exists'))
        }

        // let fileName = `./src/app/models/${inputs.name.toLowerCase()}.${inputs.type}.ts`

        // if (fs.existsSync(fileName)) {
        //     logger.error(`File ${fileName} ${t('exists, please choose another name')}`)
        //     return
        // }

        // // console.log(path.basename(process.cwd()))
        // let data = fs.readFileSync(url.getTemplate(`${inputs.type}.ts`), 'utf-8');
        // fs.mkdirSync('./src/app/models', { recursive: true });
        // fs.writeFileSync(fileName, str.replace(data, {
